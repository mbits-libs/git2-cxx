...
for sha in hashes:
    for entry in hashes[sha]:
        if is_binary(entry[0]):
            ...
            binary_file_contents[binary_file_counter] = "{{{}}}".format(
                ", ".join("0x{:02x}".format(byte) for byte in entry[0])
            )
...
for filename in sorted(text_files.keys()):
    print(
        "\t{{{}, file::{}}},".format(
            string_view(filename.encode("utf-8")), text_files[filename]
        )
    )
...
for filename in sorted(binary_files.keys()):
    print(
        "\t{{{}, span(file::{})}},".format(
            string_view(filename.encode("utf-8")), binary_files[filename]
        )
    )
