...
namespace git {
	...
	struct basic_repository : Holder {
		...
		std::string_view workdir() const { return git_repository_workdir(this->get()); }
		std::string_view gitdir() const { return git_repository_gitdir(this->get()); }
		std::string_view commondir() const { return git_repository_commondir(this->get()); }

		template <typename SubmoduleCallback>
		void submodule_foreach(SubmoduleCallback cb) const {
			git_submodule_foreach(this->get(), submodule_callback<SubmoduleCallback>,
		...
		ObjectType lookup(std::string_view id) const noexcept {
			...
			git_oid oid;
			if (git_oid_fromstrn(&oid, id.data(), id.length()))
				return {};
		...
		template <typename ObjectType>
		ObjectType lookup(git_oid const& id, size_t prefix = GIT_OID_RAWSZ) const noexcept {
			return git::create_object<ObjectType>(
				git_object_lookup_prefix,
				this->get(), &id, prefix, ObjectType::OBJECT_TYPE
				);
		}
	protected:
	...
	struct repository : basic_repository<ptr<git_repository>> {
		...
		static std::string discover(char const* start_path, Discover accross_fs = Discover::WithinFs);
