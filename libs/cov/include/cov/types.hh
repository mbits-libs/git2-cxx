...
namespace cov {
	...
	constexpr inline uint32_t MK_TAG(char c1, char c2, char c3, char c4) {
		return ((static_cast<uint32_t>(c1) & 0xFF) << 24) |
		       ((static_cast<uint32_t>(c2) & 0xFF) << 16) |
		       ((static_cast<uint32_t>(c3) & 0xFF) << 8) |
		       ((static_cast<uint32_t>(c4) & 0xFF));
	}
	...
	inline namespace v1 {
		...
		/*
		    REPORT:
		        ...
		        report_header:
		            ...
		            report_commit:
		                9:1: branch (str)
		                10:2: author (report_email)
		                12:2: committer (report_email)
		                14:1: message (str)
		                15:5: commit_id (oid)
		                20:2: committed (timestamp)
		            21:1: entries_count (uint) := EC
		            22:1: entries_offset (uint) := EO
		            23:1: entries_size (uint) := ES
		...
		*/
		...
		struct report_header {
			...
			uint32_t entries_offset;  // all offsets in u32s, from the end of
			                          // header
		...
		static_assert(sizeof(report_header) == sizeof(uint32_t[25]),
		...
		struct report_entry {
			...
			uint32_t is_modified : 1;  // 0 -- workdir file is the one tested, 1
			                           // -- workdir file was not the ont tested
			uint32_t path : 30;
