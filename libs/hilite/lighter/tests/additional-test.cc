...
namespace lighter::testing {
	TEST(additional, compare_tokens) {
		...
		ASSERT_TRUE(nl100.cmp(comment100) < 0);  // NOLINT(readability/check)
		ASSERT_TRUE(comment100.cmp(nl100) > 0);  // NOLINT(readability/check)
		ASSERT_TRUE(nl100.cmp(nl90) < 0);        // NOLINT(readability/check)
		ASSERT_TRUE(nl90.cmp(nl100) > 0);        // NOLINT(readability/check)
		ASSERT_TRUE(nl100.cmp(nl10_100) < 0);    // NOLINT(readability/check)
		ASSERT_TRUE(nl10_100.cmp(nl100) > 0);    // NOLINT(readability/check)
	}
