...
namespace cell {
	...
	template <class Subject>
	constexpr inline zero_or_more<as_parser_t<Subject>> operator*(
	...
	struct one_or_more : unary_parser<one_or_more, Subject> {
		...
		template <typename Iterator, typename Context>
		bool parse(Iterator& first, const Iterator& last, Context& ctx) const {
			if (!this->subject.parse(first, last, ctx)) return false;
	...
	template <class Subject>
	constexpr inline one_or_more<as_parser_t<Subject>> operator+(
	    Subject const& subject) noexcept {
		return {as_parser(subject)};
	}
	...
	template <class Subject>
	constexpr inline zero_or_one<as_parser_t<Subject>> operator-(
	    Subject const& subject) noexcept {
		return {as_parser(subject)};
	}
	...
	struct exactly : unary_parser<exactly, Subject> {
		constexpr exactly(const Subject& subject, std::size_t count)
		    : unary_parser<cell::exactly, Subject>(subject), count(count) {}
	...
	struct at_least : unary_parser<at_least, Subject> {
		constexpr at_least(const Subject& subject, std::size_t count)
		    : unary_parser<cell::at_least, Subject>(subject), count(count) {}
	...
	struct between : unary_parser<between, Subject> {
		constexpr between(const Subject& subject,
		                  std::size_t lower,
		                  std::size_t upper)
		...
		bool parse(Iterator& first, const Iterator& last, Context& ctx) const {
			...
			for (std::size_t n = lower; n < upper; ++n) {
				if (!this->subject.parse(first, last, ctx)) break;
			}
			return true;
		}
	};
	...
	struct between_generator {
		...
		template <typename Subject>
		constexpr between<as_parser_t<Subject>> operator()(
	...
	struct repeat_generator {
		...
		constexpr single_repeat_generator<exactly> operator()(
		    std::size_t count) const {
			return count;
		}
		constexpr single_repeat_generator<at_least> operator()(
		    std::size_t count,
		    infinity_tag) const {
			return count;
		}
		constexpr between_generator operator()(std::size_t lower,
