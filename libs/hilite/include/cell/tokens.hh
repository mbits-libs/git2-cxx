...
namespace cell {
	namespace fnv {
		...
		template <size_t bytes_in_size_t>
		struct consts {};
		template <>
		struct consts<4> {
		...
		struct consts<8> {
			...
			constexpr static const uint_fast64_t offset =
			    14695981039346656037llu;
		};
		...
		constexpr inline size_t ce_1a_hash(std::string_view view) {
			...
			for (auto sc : view) {
				const auto uc =
				    static_cast<size_t>(static_cast<unsigned char>(sc));
				hash ^= uc;
				hash *= FNV_prime;
			}
			return hash;
		...
		constexpr void sort_impl(size_t left, size_t right) {
			if (left < right) {
				...
				for (size_t i = left + 1; i < right; i++)
					if (items[i] < items[left]) swap(items[++m], items[i]);
	...
	struct set_entry {
		...
		constexpr bool operator<(const set_entry& rhs) const {
			if (hashed == rhs.hashed) return key < rhs.key;
