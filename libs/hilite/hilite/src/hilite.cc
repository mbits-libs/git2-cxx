...
namespace hl {
	namespace {
		...
		void break_token(tokens& out,
			...
			while (eol != eol_to) {
				auto const stop = eol->offset - eol->size;
				if (tok.stop <= stop) {
					out.push_back({tok.start, tok.stop, tok.kind});
					return;
				}
				out.push_back({tok.start, stop, tok.kind});
		...
		tokens break_lines(endlines::const_iterator const& eol_from,
		                   endlines::const_iterator const& eol_to,
		                   tokens::const_iterator from,
		                   tokens::const_iterator const& to) {
			tokens out;
#if 0
// enable this block to see the marker in the oom.break_lines test
			printf("break_lines :: %zu\n",
			       static_cast<size_t>(std::distance(from, to)) *
			           sizeof(tokens::value_type));
#endif
	...
	void grammar_result::produce_lines(callback& cb, size_t contents_length) {
		...
		auto broken = break_lines(std::begin(endlines_), std::end(endlines_),
		                          std::begin(tokens_), std::end(tokens_));
		std::stable_sort(std::begin(broken), std::end(broken));
		tokens_.clear();
...

		{
			...
			back.size = contents_length < back.offset
			                ? 0
			                : contents_length - back.offset;
		}
		...
		for (auto const& endline : endlines_) {
			...
			while (it != end && it->stop <= eol) {
				++token_count;
				const bool is_ws = it->kind == hl::token::whitespace;
				if (prev_ws && is_ws && it->start == prev_end) --token_count;
				prev_ws = is_ws;
				prev_end = it->stop;
				++it;
			}
			...
			while (it != end && it->stop <= eol) {
				...
				if (prev_ws && is_ws && it->start == prev_end) {
					line_tokens.back().stop = it->stop;
				} else {
					line_tokens.push_back({it->start, it->stop, it->kind});
				}
