...
namespace hl {
	...
	public:
		...
		grammar_value(grammar_result* ref, Iterator begin)
		...
		void emit(Context& context, token kind) {
			...
			auto const end = static_cast<size_t>(
	...
	template <template <class> class Value,
	          ...
	          typename Parser,
	          typename Filter>
	void parse_with_restart(Iterator begin,
	                        const Iterator& end,
	                        const Parser& code_parser,
	                        const Filter& filter_parser,
		...
		while (begin != end) {
			...
			while (begin != end &&
			       !cell::ahead(cell::eol).parse(begin, end, ctx))
				++begin;
		}
	}
}  // namespace hl
