...
namespace cell {
	...
	template <typename Iterator, typename Filter, typename Destination>
	inline void _setrange(const Iterator& first,
	                      const Iterator& last,
	                      context<Iterator, Filter, Destination>& ctx) {
	...
	template <typename Iterator, typename Filter, typename Destination>
	inline Destination& _val(context<Iterator, Filter, Destination>& ctx) {
		return ctx.dest;
	}

	template <typename Iterator, typename Filter, typename Destination>
	inline Filter const& _filter(context<Iterator, Filter, Destination>& ctx) {
		return ctx.filter;
	}
	...
	inline std::string_view _view(context<Iterator, Filter, Destination>& ctx) {
		auto const length = static_cast<size_t>(
		    std::distance(ctx.range.first, ctx.range.second));
