...
namespace cell {
	...
	struct parser : basic_parser {
		...
		constexpr action<Derived, Action> operator[](
	...
	template <class P>
	using as_parser_t = typename ext::as_parser<P>::type;
	...
	struct nary_parser : parser<Derived<Operands...>> {
		...
		constexpr Derived<Operands..., as_parser_t<Right>> append(
		    Right const& rhs) const noexcept {
			return unpacked_append(
			    rhs, std::make_index_sequence<sizeof...(Operands)>{});
		}

		template <class Right, std::size_t... Index>
		constexpr inline Derived<Operands..., as_parser_t<Right>>
		unpacked_append(Right const& rhs,
		...
		bool parse(Iterator& first, const Iterator& last, Context& ctx) const {
			return static_cast<Derived<Operands...> const*>(this)->indexed(
			    first, last, ctx,
	...
	struct action_state {
		static bool enabled() noexcept { return get_enabled(); }
		action_state() : previous_value(get_enabled()) {
			get_enabled() = false;
		}
		~action_state() { get_enabled() = previous_value; }
	...
	template <class Subject>
	constexpr inline peek_parser<as_parser_t<Subject>> ahead(
