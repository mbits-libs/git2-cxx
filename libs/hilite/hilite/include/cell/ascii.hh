...
namespace cell {
	...
	struct ascii {
		...
		static constexpr bool isascii_(int ch) { return 0 == (ch & ~0x7f); }

		static constexpr bool ischar(int ch) { return isascii_(ch); }
...
#define PREVENT_MACRO_SUBSTITUTION
	...
	struct basic_is_a : character_parser<Derived> {
		...
		/*overridable*/ constexpr bool is_a(int) const noexcept {
			return false;
		}
...
#define IS_AN(klass)                                   \
	struct is_an_##klass : basic_is_a<is_an_##klass> { \
		...
		constexpr bool is_a(int c) const noexcept {    \
			return ascii::is##klass(c);                \
		}                                              \
	};                                                 \
