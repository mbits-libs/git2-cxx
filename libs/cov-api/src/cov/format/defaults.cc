...
namespace cov {
	...
	namespace placeholder {
		namespace {
			...
			base_rating base_rating_from(cov::repository const& repo,
			                             char const* key) {
				auto value = repo.config().get_string(key);
				if (!value) {
					value = repo.config().get_string("core.rating");
				}
				if (value) {
		...
		rating environment::rating_from(cov::repository const& repo) {
			return {
			    .lines = base_rating_from(repo, "core.rating-lines"),
			    .functions = base_rating_from(repo, "core.rating-functions"),
			    .branches = base_rating_from(repo, "core.rating-branches")};
		}
	}  // namespace placeholder
