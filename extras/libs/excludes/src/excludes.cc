...
namespace cov::app::strip {
		...
		if (matches_stop(text)) {
			...
			if (inside_exclude) {
				...
				note(std::get<0>(last_start), std::get<1>(last_start),
				     "see previous start");
				include_back(std::get<0>(last_start));
			}
			last_start = {line_no, column_from(text, start_match),
			              start_match.get<0>().to_string()};
			inside_exclude = true;
		} else if (matches_line(text)) {
			is_matching_line = true;
			single_lines.push_back(line_no);
		}

		if (inside_exclude || is_matching_line) {
			exclude(line_no);
		}
	...
	void excludes::include_back(unsigned line) {
		while (!result.empty() && result.back().start >= line)
			result.pop_back();
		for (auto line_no : single_lines)
			exclude(line_no);
	}
